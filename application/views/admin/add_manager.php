...
echo $msg;
?>
<?php if(!empty($getManager)){
$photograph=$getManager[0]['image'];
}else{
  $photograph="";
}
?>
    <form role="form" action="<?php echo base_url();?>admin/add_manager/<?php echo $id;?>" method="post" enctype="multipart/form-data" >
                  <div class="box-body">
                  <input type="hidden" name="id" value="<?php if(isset($id)){echo $id;}?>">
                  <input type="hidden" name="image_old" value="<?php echo $photograph;?>">
                  <div class="form-group">
                  <label>Name:*</label>
  <input type="text" name="name" class="form-control" value="<?php  echo set_value('name');?><?php if(!empty($getManager)){echo $getManager[0]['name'];}?>"> 
                    <?php echo form_error('name', '<div class="error" style="color:red;">', '</div>');?>
                   </div>

         <div class="form-group">
         <label id="label">Designation:*</label>
        <input type="text" name="designation" class="form-control" value="<?php  echo set_value('designation');?><?php if(!empty($getManager)){echo $getManager[0]['designation'];}?>"> 
  ...
  <div id="list"><?php echo ($photograph!="") ? '<img src="'.base_url('assets/uploads/manager/'.$photograph).'" height="100" width="250">' : ''; ?></div>  
   </div>
